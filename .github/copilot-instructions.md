...
- Prompt assets live in `.devteam-source\`. Roles, Skills, `MODELS.json`, `PROVIDERS.json`, and `MCP_SERVERS.json` are loaded from there and drive model selection, BYOK provider selection, external tool access, and runtime prompting.
...
- BYOK provider profiles are declared in `.devteam-source\PROVIDERS.json`. Each entry specifies a `Name`, `Type`, `BaseUrl`, optional `ApiKeyEnvVar`, optional `BearerTokenEnvVar`, optional `WireApi`, and optional `AzureApiVersion`. Provider overrides are only supported on the Copilot SDK backend.
- The default agent backend is the GitHub Copilot .NET SDK. `CopilotSdkAgentClient` manages sessions, enables streaming, approves tools, and points the SDK at `.devteam-source\skills` via `SkillDirectories`. `CopilotCliAgentClient` remains as a fallback adapter.
...
- `WorkspaceMcpServer` exposes the workspace as a local MCP server (`devteam-workspace`). Agents call tools such as `get_workspace_summary`, `create_issue`, `remember_decision`, `spawn_agent`, `get_issue`, and `get_decisions`. `get_issue(issueId)` returns a single issue with its scoped `FilesInScope` and `LinkedDecisionIds` so agents don't need to read the full workspace. `get_decisions(decisionIds[])` returns only the decisions relevant to the current issue.
- `IssueItem` has a `RefinementState` (`Planned`, `NeedsRefinement`, `ReadyToPickup`), `FilesInScope` (list of file paths set by refinement), and `LinkedDecisionIds` (decision ids set by refinement). The orchestrator (using the `backlog-manager` skill) triages new issues: small/clear ones go directly to `ReadyToPickup`; larger or fuzzy ones get a refinement sub-issue (navigator, architect, or developer role) that populates those fields before the parent moves to `ReadyToPickup`.

## Key conventions

- Treat `.devteam-source\roles\` and `.devteam-source\skills\` as the editable behavior layer. These are markdown-first assets, not compiled code.
- Roles and Skills may declare tool expectations in frontmatter, for example `tools: rg, git, dotnet`. The loader strips frontmatter from the prompt body and stores the tool list as metadata.
...
5. **GitHub MCP** - Use the GitHub MCP extension to manage your commits and branches effectively.
